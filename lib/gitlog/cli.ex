...
#     the sha length.
  @spec parse_args([String.t]) :: { :default | :report, boolean(), String.t, String.t | nil } | :help
  def parse_args(argv) do
    { flags, params, invalid } = OptionParser.parse(argv,
    ...
    case { sorted_flags, params, invalid } do
      { [ auth: auth, cache: true, report: true ],  [ maybe_url ], [] } ->  { :report, true, maybe_url, auth }
      { [ auth: auth, report: true ],                [ maybe_url ], [] } -> { :report, false, maybe_url, auth }
      { [ auth: auth, cache: true ],                 [ maybe_url ], [] } -> { :default, true, maybe_url, auth }
      { [ cache: true, report: true ],               [ maybe_url ], [] } -> { :report, true, maybe_url, nil }
      { [ report: true ],                            [ maybe_url ], [] } -> { :report, false, maybe_url, nil }
      { [ cache: true ],                             [ maybe_url ], [] } -> { :default, true, maybe_url, nil }
      { [ auth: auth ],                              [ maybe_url ], [] } -> { :default, false, maybe_url, auth }
      { [],                                          [ maybe_url ], [] } -> { :default, false, maybe_url, nil }
      _                                                                  -> :help
    end
  end
...
"""
  ...
  def parse_user_project(string) do
    maybe_username_project_tuple = string
                                    |> String.split(".com/")
                                    |> List.last()
                                    |> String.split(".com:")
                                    |> List.last()
    ...
    case maybe_username_project_tuple do
      {username, project} -> {:ok, {username, project}}
      _ -> {:error, "Invalid github project string provided. Example: https://github.com/elixir-lang/elixir"}
    end
  end
